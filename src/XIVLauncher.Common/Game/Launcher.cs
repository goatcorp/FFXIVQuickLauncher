...
public class Launcher
{
    ...
    {
        var failed = IsBadVersionSanity(Repository.Ffxiv.GetVer(gamePath));
        failed |= IsBadVersionSanity(Repository.Ffxiv.GetVer(gamePath, true));

        if (exLevel >= 1)
        {
            failed |= IsBadVersionSanity(Repository.Ex1.GetVer(gamePath));
            failed |= IsBadVersionSanity(Repository.Ex1.GetVer(gamePath, true));
        }

        if (exLevel >= 2)
        {
            failed |= IsBadVersionSanity(Repository.Ex2.GetVer(gamePath));
            failed |= IsBadVersionSanity(Repository.Ex2.GetVer(gamePath, true));
        }

        if (exLevel >= 3)
        {
            failed |= IsBadVersionSanity(Repository.Ex3.GetVer(gamePath));
            failed |= IsBadVersionSanity(Repository.Ex3.GetVer(gamePath, true));
        }

        if (exLevel >= 4)
        {
            failed |= IsBadVersionSanity(Repository.Ex4.GetVer(gamePath));
            failed |= IsBadVersionSanity(Repository.Ex4.GetVer(gamePath, true));
        }
    ...
    private static bool IsBadVersionSanity(string text)
    {
        var failed = string.IsNullOrWhiteSpace(text);
        failed |= text.Contains("\n");
        failed |= Encoding.UTF8.GetBytes(text).All(x => x == 0x00);
