...
public class IndexVerifyCommand
{
    ...
    static IndexVerifyCommand()
    {
        ...
        COMMAND.SetHandler(x => new IndexVerifyCommand(x.ParseResult).Handle());
    }
    ...
    {
        if (this.patchRootPath is null)
            await IndexedZiPatchOperations.VerifyFromZiPatchIndex(this.patchIndexFile, this.gameRootPath);
        else
