...
public class SettingsTabPatching : SettingsTab
{
    ...
    {
        new SettingsEntry<DirectoryInfo>("Patch Path", "Where patches should be downloaded to.", () => Program.Config.GamePath, x => Program.Config.GamePath = x)
        {
            CheckValidity = x =>
            {
        ...
        new NumericSettingsEntry<long>("Maximum Speed", "Maximum download speed in bytes per second. Set to 0 for unlimited.", () => Program.Config.PatchSpeedLimit,
            x => Program.Config.PatchSpeedLimit = x, 0, long.MaxValue, 1000),
